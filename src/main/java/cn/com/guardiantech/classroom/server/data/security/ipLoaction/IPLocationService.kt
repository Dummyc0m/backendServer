                ...
                try {
                    ...
                    if (elems.size > 1){
                        ...
                        dbClient.getConnection { conn ->
                            if (conn.succeeded()) {
                                conn.result().updateWithParams("INSERT INTO `classroom_iptable` (`ip`, `geoLocation`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `geoLocation` = VALUES(`geoLocation`)", JsonArray().add(ip).add(elems[1].text()), { sql ->
                                    if (sql.failed()){
